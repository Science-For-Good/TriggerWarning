...
# Save plot function with improved file naming
...
# Create donut chart with improved visuals
create_donut_chart <- function(data, title, colors = c("#5EECC2", "#FF7400", "#8AEFD6")) {
  ...
  ggplot(data, aes(ymax = ymax, ymin = ymin, xmax = 4, xmin = 3, fill = category)) +
    geom_rect() +
    geom_text(aes(x = 2, y = labelPosition, label = label), size = 3.5, fontface = "bold") +
